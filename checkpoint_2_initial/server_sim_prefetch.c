...
void make_request_text(int id, char *buf, size_t bufsz, uint64_t *rnd) {
    bundle_prefetch_by_name("make_request_text");
...
void parse_request_text(const char *text, request_t *req) {
    bundle_prefetch_by_name("parse_request_text");    
...
int business_logic_sim(int client_id, const char *path) {
    bundle_prefetch_by_name("business_logic_sim");
...
int db_lookup_update(uint64_t key, int do_update) {
    bundle_prefetch_by_name("db_lookup_update");
...
size_t fake_compress(const char *in, size_t inlen, char *out, size_t outcap) {
    bundle_prefetch_by_name("fake_compress");
...
void *worker_fn(void *arg) {
    ...
    while (1) {
        ...
        bundle_prefetch_by_name("worker_fn");
...
void *generator_fn(void *arg) {
    bundle_prefetch_by_name("generator_fn");
    (int)(uintptr_t)arg;
...
void db_init() {
    bundle_prefetch_by_name("db_init");
...
int main(int argc, char **argv) {
    (void)argc; (void)argv;
    srand(time(NULL));
    load_bundles_json("/home/bala/Hierarchical-Software-hardware-Prefetching/checkpoint_2/bundles_pc.json");
    db_init();
