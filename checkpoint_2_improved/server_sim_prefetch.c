...
void make_request_text(int id, char *buf, size_t bufsz, uint64_t *rnd) {
    /* function-entry label for precise matching */
    void *__entry_label = &&_make_request_text_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1 /*madvise*/, 1 /*prefetch lines*/);
_make_request_text_entry:
...
void parse_request_text(const char *text, request_t *req) {
    void *__entry_label = &&_parse_request_text_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 1);
_parse_request_text_entry:
...
int business_logic_sim(int client_id, const char *path) {
    void *__entry_label = &&_business_logic_sim_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 1);
_business_logic_sim_entry:
...
int db_lookup_update(uint64_t key, int do_update) {
    void *__entry_label = &&_db_lookup_update_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 1);
_db_lookup_update_entry:
...
size_t fake_compress(const char *in, size_t inlen, char *out, size_t outcap) {
    void *__entry_label = &&_fake_compress_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 1);
_fake_compress_entry:
...
void server_log(int client_id, const char *path, int status) {
    void *__entry_label = &&_server_log_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 0); /* don't prefetch lines for logging */
_server_log_entry:
...
void *worker_fn(void *arg) {
    ...
    while (1) {
        ...
        void *__entry_label = &&_worker_fn_entry;
        bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 0);
_worker_fn_entry:
...
void *generator_fn(void *arg) {
    void *__entry_label = &&_generator_fn_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 0);
_generator_fn_entry:
...
void db_init() {
    void *__entry_label = &&_db_init_entry;
    bundles_prefetch_for_pc((uintptr_t)__entry_label, 1, 0);
_db_init_entry:
...
int main(int argc, char **argv) {
    ...
    /* initialize bundles: use absolute path you had in the original file */
    const char *bundle_path = "/home/bala/Hierarchical-Software-hardware-Prefetching/checkpoint_2/bundles_pc.json";
    if (access(bundle_path, R_OK) == 0) {
        if (bundles_init(bundle_path, 0) != 0) {
            fprintf(stderr, "Warning: bundles_init failed for %s\n", bundle_path);
        } else {
            bundles_print_stats();
        }
    } else {
        fprintf(stderr, "Warning: bundle json not found: %s\n", bundle_path);
    }
    ...
    /* cleanup (unreachable in normal server run, but good hygiene) */
    bundles_free();
